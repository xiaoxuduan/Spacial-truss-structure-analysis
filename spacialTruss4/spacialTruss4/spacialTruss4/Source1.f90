...
!DIMENSION X(NP),Y(NP),Z(NP),MM(NE),ME(ND,NE),IT(NF,NP),RR(ND,NR), NAE(NE),AE(1,2),PF(4,NCF),LMT(NDF,NE),MAXA(NPF),CKK(1000),V(NPF),DIST(NPF),PP(NPF),FF(NPF),SG(NE),SM(NE)
...
!	!ENDIF
!120    FORMAT(/6X,'节点信息'/2x,'Joint',5X,'X',5X,'Y',5X,'Z')
!121    FORMAT(1X,I4,3F8.1)
!130    FORMAT(/6X,'单元信息'/2x,'Member',2X,'START',4X,'END',6X,'NAE')
!131    FORMAT(1X,I4,3I8)
!140    FORMAT(/6X,'约束信息'/2x,'Joint',5X,'S')
!141    FORMAT(1X,I4,F8.3)
!150    FORMAT(/6X,'截面信息'/4x,'E0',8X,'A0')
!151    FORMAT(1X,1PE8.2,F8.4)
!160    FORMAT(/6X,'节点荷载'/2x,'Joint',5X,'FX',5X,'FY',7X,'FZ')
