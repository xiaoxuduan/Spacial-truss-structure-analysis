...
!    
!    
!    CHARACTER Input*20,OUTPUT*20		!主程序（读入文件，调用总计算程序，输出结果）
...
!	WRITE(12,*)'*****************************************'
!	WRITE(12,*)'*    空间桁架计算程序   *'
!	WRITE(12,*)'*      中南大学土木工程学院研桥梁1501班     *'
!	WRITE(12,*)'*      包江磊 154812240     *'
!	WRITE(12,*)'*****************************************'
!	WRITE(12,*)' '
!       WRITE(12,*)'*****************************************'
!       WRITE(12,*)'*************输入数据****************'
...
!100    FORMAT(6X,'基本信息'/2X,'NF',5X,'NP',5X,'NE',5X,'NM',5X,'NR',5X,'NCF',5X,'ND')
...
!    !common integer NF,NP,NE,NM,NR,NCF,ND,NPF,NDF
!DIMENSION X(NP),Y(NP),Z(NP),ME(ND,NE),IT(NF,NP),RR(ND,NR), NAE(NE),AE(2,NM),PF(4,NCF),LMT(NDF,NE),MAXA(NPF),CKK(10000),V(NPF),DIST(NPF),PP(NPF),FF(NPF),SG(NE),SM(NE)
!	write(*,*) 13
...
!    write(3,*) z
!    write(3,*)
!	READ(11,*)((ME(I,J),J=1,NE),I=1,2)  !MM? DELETED.
!    write(3,*) me
!    write(3,*)
...
!    write(*,*) 40
!	WRITE(12,160)
!	WRITE(12,161)(PF(1,J),PF(2,J),PF(3,J),PF(4,J),J=1,NCF)
!    
!	WRITE(12,120)
!	WRITE(12,121)(I,X(I),Y(I),Z(I),I=1,NP)
!	WRITE(12,130)
!	WRITE(12,131)(ME(1,I),ME(2,I),NAE(I),I=1,NE)
!	WRITE(12,140)
!	WRITE(12,141)(RR(1,J),RR(2,J),J=1,NR)
!	WRITE(12,150)
...
!   DO J=1,NF
!      C=C*10.0      !例如C=1.21
!      L=C+0.1       !提取C整数部分,例如L=1,即提取了约束RR(2,K)十分位上的数字,这里"+0.1"是为了防止四舍五入是出现错误
!      C=C-L
!      IF(L.EQ.0)THEN
!        N=N+1
!        IT(J,I)=N
...
!600     FORMAT(6X,'节点位移'/2x,'Joint',6X,'X(mm)',8X,'Y(mm)',6X,'Z(mm)')
!610     FORMAT(1X,I4,2X,1P3E12.2)
!620     FORMAT(//6X,'单元内力'/2x,'Member', 6X,'FN(kN)',6X,'σ(MPa)')
!630     FORMAT(3X,I4,2X,F8.2,6X,F8.2)
!640     FORMAT(//6X,'支座反力'/2x,'Joint',8X,'X',8X,'Y',8X,'Z')
